...
class TRY_SEO {
	...
	public function get_post_title( $is_default = false ) {
		global $post;

		$seo_title = ( $is_default ) ? false : trim( get_post_meta( $post->ID, 'seo_title', true ) );

		// TODO : Allow admin and REST API to call wp_get_document_title
		if( is_admin() || ( defined('REST_REQUEST') && REST_REQUEST )  ) {
			$seo_title = ( $seo_title ) ? $seo_title : get_the_title() . ' | ' . get_bloginfo('name', 'display');
		} else {
	...
	public function get_post_description( $is_default = false ) {
		global $post;
...
function try_seo_get_post_title() {
	global $try_seo;
	return $try_seo->get_post_title();
}

function try_seo_get_post_description() {
	global $try_seo;
	return $try_seo->get_post_description();
}